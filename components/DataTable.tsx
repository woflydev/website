...
const sanitizeRows = (rows: (RowArray | RowObject)[], headings: string[]): RowArray[] =>
  rows.map(row => {
...
const initializeHeadings = (
  ...
  sortable?: boolean[] | ColumnSort
): { heading: string; isSortable: boolean; sortDirection: "ascending" | "descending"; active: boolean }[] => {
...
export const DataTable: FC<DataTableProps> = ({
  ...
  const [tableHeadings, setTableHeadings] = useState(
    initializeHeadings(headings, defaultSortDirection, defaultSortColumn, sortable)
  );
  const [tableRows, setTableRows] = useState<RowArray[]>(
    sortable
      ? updateDirection(
    ...
    if (window) {
      setTimeout(() => {
        ...
        setToCssStyle(currentStyle => ({
          ...
          "--table-cell-1n": Color(base || heading)
            .alpha(0.2)
            .hsl()
            .string(),
          "--table-cell-2n": Color(base || heading)
            .rotate(-30)
            .alpha(0.15)
            .hsl()
            .string(),
          "--table-cell-hover-1n": Color(base || heading)
            .rotate(40)
            .alpha(0.18)
            .hsl()
            .string(),
          "--table-cell-hover-2n": Color(base || heading)
            .rotate(20)
            .alpha(0.15)
            .hsl()
            .string(),
          "--table-cell-border": Color(base || heading)
            .alpha(0.4)
            .hsl()
            .string(),
        }));
  ...
  if (fixedColumnWidth) {
    if (Array.isArray(fixedColumnWidth)) {
      ...
      fixedColumnWidth.forEach(
        (width, index) =>
          width && (typeof width === "string" ? (array[index] = width) : (array[index] = `${width.toString()}px`))
      );
  ...
  const onSort = e => {
    ...
    const sortDirection = active === "true" ? (direction === "ascending" ? "descending" : "ascending") : "ascending";
  ...
  return (
    <>
      ...
      <JSXStyle id={`jsx-${hashString}`}>
        {columnTextDirectionArray
          .map((val, i) => {
            return `.jsx-${hashString} td:nth-of-type(${i + 1}),.jsx-${hashString} th:nth-of-type(${
              i + 1
            }) {text-align: ${val};}`;
          })
          .join("")}
