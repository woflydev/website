        ...
        if (acc && acc[acc.length - 1] && acc[acc.length - 1].level < heading.split(" ")[0].length) {
          if (
            ...
            if (acc[acc.length - 1].subheading[acc[acc.length - 1].subheading.length - 1].subheading) {
              acc[acc.length - 1].subheading[acc[acc.length - 1].subheading.length - 1].subheading.push(
                makeSubheading(inputObj)
              );
            }
          } else {
...
const mdxOptions = {
  ...
  rehypePlugins: [
    process.env.NODE_ENV === "production" /*|| process.env.NODE_ENV === 'development'*/
      ? [require("rehype-prism-enhanced"), { showLineNumbers: true }]
